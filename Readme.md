...
標準ライブラリの拡張、コンテナ、ファイルシステム、マルチタスク、HAL等、アプリケ
ーションの部品となるモジュールを無節操に詰め合わせています。

----------------------------------------------------------------------
## 特徴

マイコンプログラム開発ではドライバ操作が大きなウエイトを占めていますが、ありがた
いことに昨今ではマイコンベンダが結構デキのいい周辺機能のデバイスドライバを提供し
てくれていることが多いです。
...
これらのプラットフォームでは、CPUごとにクセのある周辺デバイスのドライバが統一さ
れインターフェースで実装済みなので、CPUの違いをあまり意識することなく、すぐにア
プリケーション開発を行えることが特徴です。

問題は、これらのプラットフォームに対応していないボードやCPUで開発を行う場合、自
力で互換部分を実装する必要があることです。
下位レイヤが手厚い分、移植作業の手間は大きいですし、ArduinoはAVR、 mbedはARMと、
ある程度ターゲットのCPUを限定しているため、それ以外のCPUへの移植はさらに大変です
。

picoxは特定のターゲットを持たず、下位レイヤの自由度を重視しているため、mbedに
picoxを組み込むということも可能です。

ドライバ部分を自分で用意しないとこのライブラリだけでは何もできないので、玄人向け
ですが、実業務の使用にも耐えうる様、コードを吟味しています。

また、日本語ドキュメントのある低級なライブラリというのは、貴重だと思われるので低
級プログラムに興味のある組込みプログラマ初学者〜中級者の学習用途にも最適です。
