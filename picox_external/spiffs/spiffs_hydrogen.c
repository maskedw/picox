...
spiffs_file SPIFFS_open(spiffs *fs, const char *path, spiffs_flags flags, spiffs_mode mode) {
  ...
  int32_t res = spiffs_fd_find_new(fs, &fd);
  SPIFFS_API_CHECK_RES_UNLOCK(fs, res);
...
#if SPIFFS_CACHE_WR
    if (fd->cache_page) {
      offset = MAX(offset, fd->cache_page->u.s.offset + fd->cache_page->u.s.size);
    }
#endif
  }
  ...
  if ((fd->flags & SPIFFS_O_DIRECT) == 0) {
    if (len < (int32_t)SPIFFS_CFG_LOG_PAGE_SZ(fs)) {
      ...
      if (fd->cache_page) {
        ...
        if (offset < fd->cache_page->u.s.offset || // writing before cache
            offset > fd->cache_page->u.s.offset + fd->cache_page->u.s.size || // writing after cache
            offset + len > fd->cache_page->u.s.offset + SPIFFS_CFG_LOG_PAGE_SZ(fs)) // writing beyond cache page
        {
          ...
          SPIFFS_CACHE_DBG("CACHE_WR_DUMP: dumping cache page %i for fd %i:%04x, boundary viol, offs:%i size:%i\n",
              fd->cache_page->ix, fd->file_nbr, fd->obj_id, fd->cache_page->u.s.offset, fd->cache_page->u.s.size);
          res = spiffs_hydro_write(fs, fd,
              spiffs_get_cache_page(fs, spiffs_get_cache(fs), fd->cache_page->ix),
              fd->cache_page->u.s.offset, fd->cache_page->u.s.size);
      ...
      if (alloc_cpage) {
        ...
        if (fd->cache_page) {
          fd->cache_page->u.s.offset = offset;
          fd->cache_page->u.s.size = 0;
      ...
      if (fd->cache_page) {
        uint32_t offset_in_cpage = offset - fd->cache_page->u.s.offset;
        ...
        fd->cache_page->u.s.size = MAX(fd->cache_page->u.s.size, offset_in_cpage + len);
        fd->fdoffset += len;
        SPIFFS_UNLOCK(fs);
        return len;
      } else {
