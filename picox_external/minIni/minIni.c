...
static TCHAR *save_strncpy(TCHAR *dest, const TCHAR *source, size_t maxlen, enum quote_option option)
{
  ...
  X_ASSERT(maxlen>0);
  X_ASSERT(dest <= source || dest >= source + maxlen);
...
static void writesection(TCHAR *LocalBuffer, const TCHAR *Section, INI_FILETYPE *fp)
{
  if (Section != NULL && _tcslen(Section) > 0) {
    ...
    *p++ = ']';
    ...
    if (fp != NULL)
      X_ASSERT(ini_write(LocalBuffer, fp));
  } /* if */
}
...
{
  X_ASSERT(ini_close(rfp));
  X_ASSERT(ini_close(wfp));
  X_ASSERT(ini_remove(filename));
  ini_tempname(buffer, filename, INI_BUFFERSIZE);
  X_ASSERT(ini_rename(buffer, filename));
  return 1;
}
       ...
       */
        ...
        if (_tcslen(LocalBuffer) == (size_t)(tail - head)) {
          ...
          if (!ini_openrewrite(Filename, &wfp))
            return 0;
          X_ASSERT(ini_seek(&wfp, &head));
          X_ASSERT(ini_write(LocalBuffer, &wfp));
          X_ASSERT(ini_close(&wfp));
          return 1;
        } /* if */
      #endif
  ...
  if (len > 0) {
    do {
      if (!ini_read(LocalBuffer, INI_BUFFERSIZE, &rfp)) {
        ...
        if (Key!=NULL && Value!=NULL) {
          if (!flag)
            X_ASSERT(ini_write(INI_LINETERM, &wfp));  /* force a new line behind the last line of the INI file */
          writesection(LocalBuffer, Section, &wfp);
          writekey(LocalBuffer, Key, Value, &wfp);
        } /* if */
      ...
      if (!match || Key != NULL) {
        if (!cache_accum(LocalBuffer, &cachelen, INI_BUFFERSIZE)) {
          ...
          X_ASSERT(ini_read(LocalBuffer, INI_BUFFERSIZE, &rfp));
